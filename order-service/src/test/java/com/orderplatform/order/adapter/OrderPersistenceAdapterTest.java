...
class OrderPersistenceAdapterTest extends AbstractIntegrationTest {
    ...
    // Member 컨텍스트 없이 임의 memberId 사용 (MSA 분리 후 독립 테스트)
    ...
    void save_후_findById_OrderLine_포함() {
        ...
        Order order = Order.create(MEMBER_ID, lines);
    ...
    void findAllByMemberId() {
        adapter.save(Order.create(MEMBER_ID, List.of(new OrderLine(1L, "A", 1000, 1))));
        adapter.save(Order.create(MEMBER_ID, List.of(new OrderLine(2L, "B", 2000, 1))));

        List<Order> orders = adapter.findAllByMemberId(MEMBER_ID);
